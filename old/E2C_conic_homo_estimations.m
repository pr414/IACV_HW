%% Playing with conics in homogeneous coordinates
% Image Analysis and Computer Vision
% Politecnico di Milano
%
% Credits: Prof. Giacomo Boracchi
% Edits: Luca Magri. 
...
% keep in mind that for geometric problem the best is to deal with "geometric error"
%%
% Let's draw black for pixels less than zero, white for pixels greater than
% 0.
figure(1)
imshow(im > 0);
scatter(x,y, 100, 'filled');
...
% Instead of taking 5 points take just 4
% N contains in each column a generator of the null space
N = null(A(1 : 4, :));
...
% Let's assign the values to variables and build the 3x3 conic matrix
% (which is symmetrical)
% Let's start with a first solution, the first vector of the nullspace
cc = N(:, 1);
...
%% Tangent lines
% Let's draw the tangents to our points: the tangent line in homogeneous
% coordinates is l
% the line tangent to C at the line x is defined as l = C*x
...
% then plot a line of length 200 around it.
% In order to write less lines of code, I'm doing a bit of hacks with
% matrices and vectors here: theta will be a column vector of angles, and
% with a single plot command I'll plot all the lines.  Check out the
% documentation of plot to understand what's happening.
theta=atan2(-l(1,:),l(2,:));
theta = theta';
figure(1), hold on
...
%%
% Let's compute the dual conic and check that all the tangent lines we
% computed previously actually satisfy the relation (up to numeric
% precision issues).
Cstar=inv(C);
l(:,1)'*Cstar*l(:,1)
l(:,2)'*Cstar*l(:,2)
